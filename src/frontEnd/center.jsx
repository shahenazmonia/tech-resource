...
class Center extends React.Component {
    ...
    addResource(formState) {
        ...
        this.setState({resources: this.state.resources.concat(formState)});
    }
    validData(titleUrl) {
        (titleUrl.title === '' || titleUrl.url === '')
            ? alert('Bad Data!')
            : this.addResource(titleUrl);

    }
    onRemove(resource) {
        this.setState({
            resources: this.state.resources.filter((elem) => elem.title !== resource.title)
        });
    }
